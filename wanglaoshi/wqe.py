...
from typing import Dict, List, Any  # 用于类型提示，让代码更容易理解

# 数据科学和可视化库
import matplotlib.pyplot as plt  # Python最流行的绘图库
import matplotlib.font_manager as fm  # 字体管理器
from matplotlib import rcParams  # matplotlib配置参数
import numpy as np  # 数值计算库
import pandas as pd  # 数据处理库
import seaborn as sns  # 统计图表库
from jinja2 import Environment, FileSystemLoader  # 模板引擎，用于生成报告
from scipy.stats import skew, kurtosis, zscore, normaltest, shapiro  # 统计分析函数
from sklearn.preprocessing import StandardScaler  # 数据标准化工具
from sklearn.decomposition import PCA  # 主成分分析工具
from statsmodels.stats.outliers_influence import variance_inflation_factor  # 方差膨胀因子计算

# 配置日志系统 - 记录程序运行过程中的重要信息
logging.basicConfig(
    level=logging.INFO,  # 设置日志级别为INFO（显示一般信息）
...
class DataProcessor:
    """数据处理器 - 负责处理各种数据转换任务的小工具集合"""

    @staticmethod
    def validate_dataframe(df: pd.DataFrame) -> None:
        """验证输入的数据是否有效
        这个函数会检查传入的是否是pandas DataFrame对象，以及是否为空
        """
        if not isinstance(df, pd.DataFrame):
            raise TypeError("输入必须是pandas DataFrame")  # 抛出错误，提示用户输入类型不对
        if df.empty:
            raise ValueError("DataFrame不能为空")  # 抛出错误，提示用户数据不能为空
    ...
    def get_column_types(df: pd.DataFrame) -> Dict[str, List[str]]:
        ...
        """
        return {
            'numeric_cols': df.select_dtypes(include=['number']).columns.tolist(),      # 数字列
            'categorical_cols': df.select_dtypes(include=['object', 'category']).columns.tolist(),  # 分类列
            'datetime_cols': df.select_dtypes(include=['datetime']).columns.tolist()   # 时间列
        }

    @staticmethod
    def process_value(v: Any) -> Any:
        """处理单个数据值，使其能够被JSON序列化
        因为有些数据类型不能直接转换为JSON格式，需要特殊处理
        """
        if isinstance(v, np.ndarray):
            return v.tolist()  # numpy数组转为Python列表
        if isinstance(v, (np.int64, np.int32)):
            return int(v)  # numpy整数转为Python整数
        if isinstance(v, (np.float64, np.float32)):
            if np.isnan(v) or np.isinf(v):
                return None  # NaN和无穷大值替换为None
            return float(v)  # numpy浮点数转为Python浮点数
        if isinstance(v, datetime):
            return v.isoformat()  # 日期时间转为ISO格式字符串
        if isinstance(v, pd.Series):
            return v.to_dict()  # pandas Series转为字典
        if isinstance(v, pd.DataFrame):
            return v.to_dict('records')  # pandas DataFrame转为记录列表
        try:
            if pd.isna(v):
                return None  # pandas的缺失值转为None
    ...
    def process_data(obj: Any) -> Any:
        """递归处理复杂数据结构，确保都能被JSON序列化"""
        if isinstance(obj, dict):
            return {k: DataProcessor.process_data(v) for k, v in obj.items()}  # 处理字典
        elif isinstance(obj, (list, np.ndarray)):
            return [DataProcessor.process_data(item) for item in obj]  # 处理列表或数组
        else:
            return DataProcessor.process_value(obj)  # 处理单个值

    @staticmethod
    def chunk_dataframe(df: pd.DataFrame, chunk_size: int = 1000) -> List[pd.DataFrame]:
        """将大数据表分割成小块处理，避免内存不足"""
        return [df[i:i + chunk_size] for i in range(0, len(df), chunk_size)]  # 每1000行为一块
...
class DataAnalyzer:
    ...
    def __init__(self, df: pd.DataFrame):
        ...
        self.df = df.copy()  # 创建数据副本，防止修改原始数据

        # 识别数据中各列的类型
        column_types = DataProcessor.get_column_types(self.df)
        self.numeric_cols = column_types['numeric_cols']      # 数字列
        self.categorical_cols = column_types['categorical_cols']  # 分类列
        self.datetime_cols = column_types['datetime_cols']    # 时间列
        ...
        # 初始化缓存，提高性能
        self._correlation_cache = None     # 相关性分析缓存
        self._multicollinearity_cache = None  # 多重共线性分析缓存
        self._memory_usage = None         # 内存使用情况缓存

    def __del__(self):
        """析构函数，在对象销毁时清理资源"""
        if hasattr(self, 'visualizer'):
            self.visualizer.clear_cache()  # 清理图表缓存

    def correlation_analysis(self) -> pd.DataFrame:
        """相关性分析 - 分析数值列之间是否存在关联关系"""
    ...
    def multicollinearity_analysis(self) -> pd.DataFrame:
        """多重共线性分析 - 检查数值列之间是否存在高度相关性（可能导致建模问题）"""
        if self._multicollinearity_cache is None:
            ...
            numeric_df = numeric_df.fillna(numeric_df.mean())  # 用平均值填充缺失值

            # 标准化数据（使不同量纲的数据具有可比性）
            scaler = StandardScaler()
            scaled_data = scaler.fit_transform(numeric_df)

            # 计算方差膨胀因子(VIF)，衡量多重共线性程度
    ...
    def plot_distribution(self, column: str) -> str:
        ...
        if column not in self.df.columns:
            raise ValueError(f"列名不存在: {column}")  # 检查列是否存在
        return self.visualizer.plot_distribution(self.df[column], column)  # 调用可视化工具绘图

    def plot_correlation_heatmap(self) -> str:
        """绘制相关性热力图 - 用颜色深浅表示各列之间的相关性强弱"""
    ...
    def generate_report(self, output_html: str = "analysis_report.html") -> None:
        """生成完整的分析报告 - 将所有分析结果汇总成一个HTML网页报告"""
        try:
            # 获取数据集名称
            try:
            ...
            analysis_results = {
                ...
                "basic_stats": self.basic_statistics().to_dict('records'),           # 基础统计
                ...
                "duplicate_analysis": self.duplicate_analysis(),                      # 重复值分析
                "correlation_matrix": self.correlation_analysis().to_dict(),          # 相关性分析
                "multicollinearity": self.multicollinearity_analysis().to_dict('records'), # 多重共线性分析
                "pca_analysis": self.pca_analysis(),                                  # 主成分分析
                "plots": {
                    "distribution": {col: self.plot_distribution(col) for col in self.df.columns}, # 各列分布图
                    "correlation": self.plot_correlation_heatmap()                   # 相关性热图
                },
                "interpretations": self.interpret_results()                           # 结果解读
            }
    ...
    def normality_test(self) -> pd.DataFrame:
        """正态性检验 - 检查数据是否符合正态分布（很多统计方法要求数据正态分布）"""
        results = []
    ...
    def interpret_results(self) -> Dict[str, Any]:
        """解读所有分析结果 - 将复杂的统计结果转化为易于理解的文字说明"""
        interpretations = {
            "basic_info": self._interpret_basic_info(),           # 基本信息解读
            "data_quality": self._interpret_data_quality(),       # 数据质量解读
            "statistical_analysis": self._interpret_statistical_analysis(),  # 统计分析解读
            "recommendations": self._generate_recommendations()   # 处理建议
        }
    ...
    def plot_distribution(self, data: pd.Series, column: str) -> str:
        ...
        try:
            ...
            if pd.api.types.is_numeric_dtype(data):
                sns.histplot(data, kde=True)  # 数值型数据绘制直方图和密度曲线
                ...
                sns.countplot(x=data)  # 分类型数据绘制条形图
